...
function Initialize (context)
	context.Name = "Fast Two column separation"
	context.DisplayName = "Two column separation"
	context.LegacyName = "Two column separation"
	...
	context:DeclareASParameter("Pump B Starting Pressure Offset", -40.0, "%", "decimal")
...
function GenerateMethod (experiment, installed, context)
	...
	context:SetArgumentValue("trap_equilibration_volumemultiplier", 40)
	context:SetArgumentValue("separator_equilibration_volumemultiplier", 40)
	...
	pressure = pf.getMaxColumnPressure(experiment.Trap, experiment.Separator, pressSettings.GradientPumpMaxTargetPressure)
...
function Main (installed, context)
	...
	local separationMode = "In fast two column: "
	local pressSettings = pf.getPressureSensorSettings(installed.MaxPumpPressure)
...
Check that the instrument is powered on and connected. Try restarting the instrument and running this procedure again.")
	...
	if equilibrateAtTheGradientEnd then
		local equilibrationPressure = context:GetArgumentValue("separator_equilibration_pressure")
		...
		pf.Manualmode_Pump_constantPressure(zr.A, equilibrationPressure, pump, sleep_100)
		while math.abs(pump:GetCurrentPressure(zr.A) - equilibrationPressure) > 2 do sleep_100() end
	else
		pf.reducePressure(context, pump, zr, zr.A, zr.B, 15, 10, 20, sleep_250, baltic.smooth)
...
--		local waitTime = pf.now() + delay_InjectSample
	...
	---@return string
	---@return string
	local function equilibration(yield_func)
		local err = ""
		local msg = ""
		...
		if context:GetArgumentValue("Exclude Equilibration") == true then
			...
			local additionalTime = 25
			if pump:GetCurrentPressure(zr.A) > 100 then additionalTime = 35 end
			local txt = DotNetString.Format(", this finishes at about: {0:#0.00} min.", (elapsedTime+a_sE.equilibratingTime+additionalTime)/60)
			status:SetStatus(baltic.Status.EquilibrateSeparator..txt)
			err, msg = se.sepEquilibration(installed, context, a_sE, ovenTemp, false, yield_func)
			status:RemoveStatus(baltic.Status.EquilibrateSeparator..txt)
			if ( err ~= "" ) then
				context:Log("--- Separator Equilibration failed")
				msg = "Separation column equilibration - "..msg
				handle_message()
			end

			pr.Signalize_Reset(context)

			pf.Manualmode_Pump_constantPressure(zr.A, a_tE.equilibratingPress, pump, yield_func)
			local actPressA = pump:GetCurrentPressure(zr.A)
			local timeOut = 600
			while ((actPressA - 5) > a_tE.equilibratingPress) and (timeOut > 0) do
				parallel.sleep(yield_func, 100)
				actPressA = pump:GetCurrentPressure(zr.A)
				timeOut = timeOut - 1
			end
			elapsedTime = pf.now()-startTime
			txt = DotNetString.Format(", this finishes at about: {0:#0.00} min.", (elapsedTime+a_tE.equilibratingTime)/60)
			status:SetStatus(baltic.Status.EquilibrateTrap..txt)
			zr.ChangePressurePID(context, pump, baltic.PPID.PID_20_3_1)
	...
	local p_equilibration = { equilibration, parallel.yield }
	...
	error_code.err, error_code.message, offsetA, offsetB = parallel.run(sleep_100, p_equilibration, p_injection, p_logPressSensorOffsetA, p_logPressSensorOffsetB)
	sleep_250()
...
--				settings.FlowCalibrationOffsetA = offsetA
		journal:Add(JournalEntry.Set(LogTo.Twin, context.Name, "Flow Sensor A Offset Fast2C Equilibration", offsetA, "\181L/min", "N4"))
	end
	if offsetB ~= nil then
--	Don't save the new offset. Only report it.				
--				pf.SetFlowCalibrationOffset(zr.B, offsetB, pump, sleep_200)
--				settings.FlowCalibrationOffsetB = offsetB
		journal:Add(JournalEntry.Set(LogTo.Twin, context.Name, "Flow Sensor B Offset Fast2C Equilibration", offsetB, "\181L/min", "N4"))
	end
